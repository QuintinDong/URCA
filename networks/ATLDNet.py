...
class ATLDNet(nn.Module):

    def __init__(self, feature_scale=4, n_classes=21, is_deconv=True, in_channels=3, is_batchnorm=True):
        super(ATLDNet, self).__init__()
        ...
        self.final_seg = nn.Conv3d(filters[0], n_classes, 1)
        self.final_edge = nn.Conv3d(filters[0], n_classes, 1)
    ...
    def forward(self, inputs):
        ...
        final_tanh = self.tanh(final)

        final_seg = self.final_seg(up1)

        final = self.final_edge(up1)
        final_edge = 1 - self.relu(self.tanh(final))
        return final_tanh, final_seg, final_edge
