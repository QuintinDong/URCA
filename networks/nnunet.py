...
class Generic_UNet(SegmentationNetwork):
    ...
    def forward(self, x):
        ...
        for u in range(len(self.tu)):
            x = self.tu[u](x)
            #print(x.shape)#torch.Size([2, 320, 24, 4, 4])
            #print(skips[-(u + 1)].shape)#torch.Size([2, 320, 24, 3, 3])
            # trans_resize=transforms.Resize((3,3))
            # x=trans_resize(x)
            # x = F.interpolate(x, size=(3, 3), mode='trilinear', align_corners=True)
            pool = nn.AdaptiveAvgPool3d((24, 3, 3))
            x=pool(x)#[1,320,24,3,3]
            #print(x.shape)
