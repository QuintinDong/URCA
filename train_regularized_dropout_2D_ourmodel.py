...
parser.add_argument('--exp', type=str,
                    default='ACDC/Regularized_Dropout_ourmodel', help='experiment_name')
...
def train(args, snapshot_path):
    ...
    optimizer2 = optim.SGD(model2.parameters(), lr=base_lr,
                           momentum=0.9, weight_decay=0.0001)
    optimizer_ema = optim.SGD(model_ema.parameters(), lr=base_lr,
                            momentum=0.9, weight_decay=0.0001)
    ...
    for epoch_num in iterator:
        for i_batch, sampled_batch in enumerate(trainloader):
            ...
            outputs1_labeled = model1(volume_batch[:args.labeled_bs])
            outputs1_labeled_soft1 = torch.softmax(outputs1_labeled,dim =1)



            noise = torch.clamp(torch.randn_like(volume_batch[args.labeled_bs:]) * 0.1, -0.2, 0.2)
            ema_inputs = volume_batch[args.labeled_bs:] + noise  # 加入噪声

            with torch.no_grad():
                outputs2 = model2(volume_batch)
                outputs_soft2 = torch.softmax(outputs2, dim=1)
                outputs_ema = model_ema(volume_batch)
                outputs_ema_unlabeled = model_ema(ema_inputs)
            ...
            #---------todo-----------生成不确定性
            T = 8
            _, _, w, h = volume_batch[args.labeled_bs:].shape
            volume_batch_unlabeled = volume_batch[args.labeled_bs:].repeat(2, 1, 1, 1)
            #print(volume_batch_unlabeled.shape)#torch.Size([12, 1, 256, 256])
            stride = volume_batch_unlabeled.shape[0] // 2
            preds = torch.zeros([stride * T, num_classes, w, h]).cuda()  # 返回一个由标量0填充的张量，形状由size决定
            # preds_1 = torch.zeros([stride * T, num_classes, w, h]).cuda()
            for i in range(T // 2):
                ema_inputs = volume_batch_unlabeled + torch.clamp(torch.randn_like(  # torch.randn_like返回一个与输入张量大小相同的张量，其中填充了均值为 0 方差为 1 的正态分布的随机值。
                                     volume_batch_unlabeled) * 0.1, -0.2,0.2)  # clamp（）函数的功能将输入input张量每个元素的值压缩到区间 [min,max]，并返回结果到一个新张量
                with torch.no_grad():
                    preds[2 * stride * i:2 * stride * (i + 1)] = model2(ema_inputs)
                    #preds_1[2 * stride * i:2 * stride * (i + 1)] = model_ema(ema_inputs)
            preds = F.softmax(preds, dim=1)
            preds = preds.reshape(T, stride, num_classes, w, h)
            preds = torch.mean(preds, dim=0)
            uncertainty = -1.0 * torch.sum(preds * torch.log(preds + 1e-6), dim=1, keepdim=True)

            # preds_1 = F.softmax(preds_1, dim=1)
            # preds_1 = preds_1.reshape(T, stride, num_classes, w, h)
            # preds_1 = torch.mean(preds_1, dim=0)
            # uncertainty_1 = -1.0 * torch.sum(preds_1 * torch.log(preds_1 + 1e-6), dim=1, keepdim=True)

            threshold = (0.75 + 0.25 * ramps.sigmoid_rampup(iter_num, max_iterations)) * np.log(2)
            threshold_1 = (0.78 + 0.25 * ramps.sigmoid_rampup(iter_num, max_iterations)) * np.log(2)

            mask = (uncertainty < threshold).float()

            mask_1 = (uncertainty < threshold_1).float()

            uncertainty_volume_batch_unlabeled = torch.matmul(mask,volume_batch[args.labeled_bs:])

            outputs1_uncertainty = model1(uncertainty_volume_batch_unlabeled)

            with torch.no_grad():

                outputs2_uncertainty = model2(uncertainty_volume_batch_unlabeled)
                outputs_soft2_uncertainty = torch.softmax(outputs2_uncertainty, dim=1)

                outputs_ema_unlabeled_uncertainty = model_ema(uncertainty_volume_batch_unlabeled)
                outputs_soft_ema_uncertainty = torch.softmax(outputs_ema_unlabeled_uncertainty, dim=1)

            #print(outputs1_labeled[:args.labeled_bs].shape)#torch.Size([6, 4, 256, 256])
            #print(label_batch[:args.labeled_bs].shape)#torch.Size([6, 256, 256])
            ...
            #new_label_1 = torch.add(outputs1[:args.labeled_bs],label_batch[:args.labeled_bs].unsqueeze(1))
            ...
            model_ema_loss = 0.5 * (ce_loss(outputs_ema[:args.labeled_bs], label_batch[:args.labeled_bs].long()) + dice_loss(
                outputs_soft_ema[:args.labeled_bs], label_batch[:args.labeled_bs].unsqueeze(1)))


            #[12,4,256,256]  [12,256,256]  [12,4,256,256]   [12,256,256]


            #----------todo-------------------真标签与伪标签相加生成新的标签---->求loss
            #model1_new_label_loss = losses.softmax_dice_loss(outputs1_labeled, new_label_1)

            #[6,4,256,256]   [6,4,256,256]   [6,4,256,256]   [6,4,256,256]




            r_drop_loss = losses.compute_kl_loss(outputs1[args.labeled_bs:], outputs2[args.labeled_bs:])
            r_drop_loss_1 = losses.compute_kl_loss(outputs1[args.labeled_bs:], outputs_ema_unlabeled)
            r_drop_loss_2 = losses.compute_kl_loss(outputs2[args.labeled_bs:], outputs_ema_unlabeled)


            # uncertain_loss_1 = losses.compute_kl_loss(outputs1_uncertainty, outputs2_uncertainty)
            # uncertain_loss_2 = losses.compute_kl_loss(outputs1_uncertainty, outputs_ema_unlabeled_uncertainty)
            # uncertain_loss_3 = losses.compute_kl_loss(outputs2_uncertainty, outputs_ema_unlabeled_uncertainty)


            # consistency_loss = torch.sum(mask * r_drop_loss_1) / (2 * torch.sum(mask) + 1e-16)
            #
            # consistency_loss_1 = torch.sum(mask_1 * r_drop_loss_2) / (2 * torch.sum(mask_1) + 1e-16)


            loss = model1_loss + model2_loss + model_ema_loss  + consistency_weight * r_drop_loss + consistency_weight * r_drop_loss_1 + consistency_weight * r_drop_loss_2
                   # + consistency_weight * consistency_loss  + \
                   # consistency_weight * consistency_loss_1




            optimizer1.zero_grad()#梯度参数置零
            ...
            optimizer1.step()#根据每个参数的梯度和学习率等超参数进行参数的更新
            # optimizer2.step()
            # optimizer_ema.step()


            #update_ema_variables_co(model1, model2, model_ema, args.ema_decay, iter_num)
            update_ema_variables(model1,model2,args.ema_decay, iter_num)
            update_ema_variables(model2, model_ema, args.ema_decay, iter_num)
            ...
            if iter_num > 0 and iter_num % 200 == 0:
                ...
                for i_batch, sampled_batch in enumerate(valloader):
                    metric_i = test_single_volume(
                        sampled_batch["image"], sampled_batch["label"], model_ema, classes=num_classes)
                    metric_list += np.array(metric_i)
                metric_list = metric_list / len(db_val)
                for class_i in range(num_classes - 1):
                ...
                writer.add_scalar('info/ema_model_val_mean_asd', mean_asd3, iter_num)
                writer.add_scalar('info/ema_model_val_mean_hd', mean_hd3, iter_num)
                writer.add_scalar('info/ema_model_val_mean_jc', mean_jc3, iter_num)
                ...
                if performance3 > best_performance3:
                    ...
                    torch.save(model_ema.state_dict(), save_mode_path)
                    torch.save(model_ema.state_dict(), save_best)

                logging.info(
                    'iteration %d : ema_model_mean_dice : %f model2_mean_hd95 : %f model2_mean_asd: %f model2_mean_hd: %f model2_mean_jc: %f' % (
